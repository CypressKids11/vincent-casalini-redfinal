...
const getRealImageDescription = async (image) => {
  const response = await fetch(
    ...
    {
      headers: {
        Authorization: "Bearer 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",
      },
      method: "POST",
      body: image,
    }
...
async function doOperation() {
  ...
  rightIsAI = false;
  nextButton.disabled = true;
  getRealImage().then(async (realIamge) => {
    let randomBox = getRandomBox();
    getRealImageDescription(realIamge).then(async (imageDesc) => {
      descriptionToImage({ "inputs": imageDesc[0].generated_text }).then(
        async (blob) => {
          let base64 = await toBase64(blob);
          leftImage.classList.remove("loading")
          rightImage.classList.remove("loading")
          randomBox.childNodes[1].src = realIamge;
